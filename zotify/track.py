...
class Config:
    ...
    def get_credentials_location(cls) -> str:
        ...
        return str(credentials_location)

    @classmethod
    def get_temp_download_dir(cls) -> str:
        if cls.get(TEMP_DOWNLOAD_DIR) == '':
            return ''
        return str(PurePath(cls.get_root_path()).joinpath(cls.get(TEMP_DOWNLOAD_DIR)))
    ...
    def get_output(cls, mode: str) -> str:
        ...
        if mode == 'playlist':
            if cls.get_split_album_discs():
                ...
                return str(PurePath(split).joinpath('Disc {disc_number}').joinpath(split))
